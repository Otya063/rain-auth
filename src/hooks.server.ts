...
import { getToken, withDb } from '$lib/utils/server';
...
export const handle: Handle = async ({ event, resolve }) => {
    ...
    if (!connectionString) {
        return new Response('HYPERDRIVE_UNDEFINED', { status: 500 });
    }

    // DB接続はリクエストごとに生成する（Workersはリクエストを跨いだI/Oを許可しない）
    return withDb(connectionString, event.platform?.ctx, async () => {
